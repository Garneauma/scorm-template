...
- Single HTML page (`m0_en.html`) to demonstrate how to include Bootstrap.
...
Open <http://localhost:3100> to see the page in action.
...
| `css-purge` | Runs [PurgeCSS](https://purgecss.com) to remove CSS that is unused by `m0_en.html` |
...
We've included a single npm script that runs PurgeCSS against our single `m0_en.html` file to remove unused styles from `assets/css/starter.css`.

To purge your CSS, run `npm run css-purge` from the command line. This executes the following:

```shell
npm purgecss --css assets/css/starter.css --content m0_en.html --output assets/css/
```
